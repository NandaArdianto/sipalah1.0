        ...
        <ul class="sidebar-nav" id="sidebar-nav">
            ...
            <li class="nav-item">
                ...
                <ul id="tables-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
                    ...
                    <li>
                        <a href="{{ route('dashboard.laporan.validasi') }}" class="active">
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Tabel Validasi</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
                                ...
                                <tbody>
                                    @foreach ($reports as $report)
                                        <tr>
                                            ...
                                            {{ $report->status == 'approved' ? 'bg-success text-white' : ($report->status == 'rejected' ? 'bg-danger text-white' : ($report->status == 'pending' ? 'bg-warning text-dark' : '')) }}">
                                            ...
                                            <td>
                                                @if ($report->status == 'pending')
                                                    <form action="{{ route('reports.validate', $report->id) }}"
                                                        method="POST" style="display:inline-block; margin:5px;">
                                                        @csrf
                                                        <button type="submit" class="btn btn-success btn-sm"
                                                            name="status" value="approved">Approved</button>
                                                    </form>
                                                    <form action="{{ route('reports.validate', $report->id) }}"
                                                        method="POST" style="display:inline-block; margin:5px;">
                                                        @csrf
                                                        <button type="submit" class="btn btn-danger btn-sm" 
                                                            name="status" value="rejected">Rejected</button>
                                                    </form>
                                                @else
                                                    {{ ucfirst($report->status) }}
                                                @endif
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                validateModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    const reporterEmail = button.getAttribute('data-email');
                    const reporterName = button.getAttribute('data-name');
                    const currentStatus = button.getAttribute('data-status');

                    // Set form action menggunakan route helper
                    const form = document.getElementById('validateReportForm');
                    form.action = `/reports/validate/${reportId}`;

                    // Atur status saat ini (optional)
                    // Jika Anda ingin menampilkan status saat ini, Anda bisa melakukannya di sini
                    ...
                    // Jika status saat ini sudah diset, Anda bisa menyesuaikannya
                    // Misalnya, menampilkan status saat ini atau disable opsi tertentu
                });
